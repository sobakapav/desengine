...
- **THEN** workflow-действия `start`, `iterate` и `check` используют новый активный провайдер без изменений в коде
...
- **WHEN** инициирующий запуск workflow не получил ответ от активного провайдера за настроенный timeout
- **THEN** система прерывает вызов
- **AND** сохраняет текущее состояние проекта без частичных изменений
...
- **WHEN** проверка workflow-результата не получила ответ от активного провайдера за настроенный timeout
- **THEN** система прерывает вызов
- **AND** сохраняет текущее состояние проекта без частичных изменений
...
- **WHEN** в проектной работе накоплены учебная стоимость и реальные метрики провайдера
...
#### Scenario: Инициирующий запуск workflow превышает runtime payload budget
