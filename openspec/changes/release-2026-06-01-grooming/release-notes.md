...
- Если модель на старте уровня вернула имя файла вместо кода, запуск теперь восстанавливается безопасно и не срывает рабочий набор.
- Уточнения и проверка уровня больше не должны зависать бесконечно: система возвращает понятную ошибку и позволяет сразу повторить действие.
...
- Sandpack preview теперь надёжнее показывает именно стилизованный результат, а не «голый» DOM без применённого CSS.
