...
В active OpenSpec уже есть runtime/spec-основание для Workbench и связанного workflow-контракта. Producer-уровень теперь закреплён в `producer-workbench`: Workbench считается materialized и пока locked рабочей поверхностью продукта. Этот dispatcher не заменяет producer и не создаёт новую параллельную capability, а задаёт тактическую рамку для дальнейшего развития workbench-линии внутри `focus-domain`.
...
- Удержать связь Workbench ↔ Project ↔ Workflow ↔ Subject как основу downstream runtime-изменений.
...
- Принять producer-контекст, что Workbench уже materialized, но пользовательская работа на нём пока закрыта, а `level-labs` — не долгосрочная модель.
...
- `WorkbenchDefinition`:
  - `id`
  - `title`
  - `applicability` (для каких `subject.kind` и/или `workflowStep.kind`)
...
- получать входные артефакты из Project/Workflow;
- производить выходные артефакты и сохранять их обратно в проектный контур;
...
- открыть проект → открыть верстак текущего workflow/subject;
...
- Unit/contract: выбор верстака по проекту/subject/шагу, сериализация состояния, применимость инструментов.
- Component/browser: базовые интеракции верстака и 1–2 инструментов, включая layout и image-inspector tools.
- E2E smoke: открыть проект → открыть верстак → выполнить действие → артефакт сохранился.
