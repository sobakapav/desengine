...
`dispatcher-dataflow` удерживает форму события и более широкий контур передачи данных. Но сама система журналирования требует отдельного управленческого слоя: даже без немедленной реализации нужно заранее договориться, чем является product event log и чего от него не ждать на первом этапе.
...
3. Product event log рассматривается как отдельный слой рядом с общим event contract:
   - `dispatcher-dataflow` отвечает за форму события и dataflow-границы;
