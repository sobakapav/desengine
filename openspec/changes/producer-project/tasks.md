...
- Пока в активной реализации удерживаем только цепочку `проект -> компоненты -> workflow -> работа`.
- Внутри project-линии это значит:
  - `ProjectWorkspace` и active project boundary;
  - project-aware component/workflow контекст;
  - читаемый переход из проекта в работу и обратно;
  - влияние project contract на рабочие линии, workflow и итоговую согласованность.
...
- [ ] 3. Зафиксировать, что ближайший downstream behavior-change — отдельный `implement`-change для `project entity and storage boundary`.
  ...
  - [ ] 3.4 change не включает полную project-scoped миграцию component/workflow/workbench и progress.
- [ ] 4. Зафиксировать влияние project-level `UI kit` на существующие сущности:
  - [ ] 4.1 component/workflow слой начинает жить внутри project contract;
...
- [x] 6. Развести границы первой волны и будущих направлений:
  ...
  - [x] 6.2 определить точную MVP decomposition как набор downstream waves:
    ...
    - [x] `implement-project-component-workflow-entrypoint` (`implement`);
  ...
  - [x] 6.3 отделить project entity, component entrypoint, workflow binding, workbench binding и progress invalidation как разные delivery-вопросы с запретом на их обратное слияние без нового producer-level решения.
- [x] 7. Обеспечить тестовую и traceability-готовность будущих behavior-change changes:
  - [x] 7.1 указать ожидаемые уровни проверки для foundation, component, workflow, workbench и migration waves:
    - [x] foundation: `static/contract` + `unit`;
    - [x] component: `static/contract` + `unit`;
...
- Затронутые OpenSpec capability/scenarios:
  ...
  - `projects`: последующий перенос component/workflow смысла внутрь project contract после foundation-слоя;
...
- Mock/fixture-данные:
  ...
  - downstream changes обязаны явно зафиксировать fixture/mocks отдельно для:
    ...
    - project-aware component/workflow fixtures;
...
- Downstream verification matrix, которую producer обязан задать заранее:
  ...
  - component-wave: `npm run test:traceability` + `npm run test:unit -- <project-component-workflow-entrypoint-tests>`;
