...
Пока active delivery идёт вокруг основной цепочки `проект -> workflow -> проверка/чеклист -> результат`, dispatcher считает приоритетными только:

- шаги и переходы, которые помогают открыть работу из проекта;
- workflow language, который делает проверку и результат понятными;
