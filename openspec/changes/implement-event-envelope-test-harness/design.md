...
4. Первый заход фиксирует только foundation baseline. Producer-specific payload samples и сценарии `experience` / `action` / `cost` не обязательны, пока они не нужны для проверки общего envelope или общего log boundary.

5. `coverage-plan.json` нужен не по факту отсутствия producer tests, а только по факту осознанного gap в заявленном foundation coverage.
