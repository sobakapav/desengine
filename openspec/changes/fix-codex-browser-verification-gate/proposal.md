...
- `os:close` для `component/browser` changes тоже может идти через прямой Playwright path;
...
- Научить `os:close` автоматически переводить `component/browser` verification на wrapper-path вместо прямого `npm run test:e2e -- test/e2e/*.spec.ts`.
...
- `component/browser` fixes будет проще честно закрывать и труднее ложно блокировать неподходящим execution mode.
