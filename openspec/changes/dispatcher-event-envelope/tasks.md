...
- [ ] 1. Зафиксировать роль `dispatcher-event-envelope` как управляющего контура общего event contract.
- [ ] 2. Определить обязательное ядро `EventEnvelope` и допустимые scope-комбинации.
- [ ] 3. Зафиксировать границу между envelope и payload profiles.
- [ ] 4. Перечислить downstream changes, которые обязаны использовать общий envelope.
- [ ] 5. Развести по ответственности `dispatcher-event-envelope` и `dispatcher-log-system`.
