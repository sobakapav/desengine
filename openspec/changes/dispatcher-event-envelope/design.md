...
Продюсерский change по `EventEnvelope` уже определил продуктовую мотивацию и рамки. Следующий шаг не в коде, а в техническом управлении внедрением: нужен единый dispatcher под `focus-tech`, который удерживает общий контракт события как foundation-границу для нескольких будущих подсистем и при этом явно несёт producer-контекст через `producer_ref`.
