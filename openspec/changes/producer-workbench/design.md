...
До стабилизации основной цепочки `проект -> workflow -> проверка/чеклист -> результат` эта линия не форсирует:
...
Workbench сейчас рассматривается только как поверхность, которая помогает пройти текущую работу, отправить результат на проверку и увидеть итог.
