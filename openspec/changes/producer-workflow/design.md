...
- Не навязывать один конкретный workflow всем типам задач.
...
- task и artifacts живут внутри этого пути, а не рядом с ним;
...
2. `Task` задаёт цель и артефакты.
...
- Один workflow всегда материализуется последовательностью шагов или возможны фазы с вложенными подзадачами.
- Где проходит граница между workflow transitions и task status transitions.
