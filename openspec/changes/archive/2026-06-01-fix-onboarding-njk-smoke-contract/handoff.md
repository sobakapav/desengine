...
- Какой guard обязателен в результате: поведенческий unit/integration guard должен явно прогонять `validateOnboardingLayout` и `inspectOnboardingState` для `tools/smoke-local-install/onboarding.mjs`, а также `validateOnboardingLayout` для `tools/repair-onboarding.mjs`, чтобы возврат к ожиданию `default.md` ловился по фактическому состоянию layout/source, а не только по строковому source-contract.
...
- Дополнительная обязательная проверка: `npm run test:unit -- test/unit/onboarding-prompt-templates.test.ts test/unit/p2-source-contracts.test.ts`
- Что именно должен доказать результат проверки: smoke и repair больше не падают на валидном onboarding-layout только из-за ожидания `default.md`; поведенческий unit guard отдельно доказывает, что `inspectOnboardingState` не возвращает `synced` для legacy-only layout с `default.md`, а подтверждает `synced` только для полного layout с каноническим `default.njk`; preflight либо подтверждает корректный onboarding, либо сообщает реальную проблему layout/source.

## Открытые вопросы

- Нужен ли в следующем шаге общий runtime/CLI helper для layout validation, если текущего behavioural guard и существующего source-contract слоя окажется недостаточно.
