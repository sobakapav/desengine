...
#### Scenario: Ночная волна fixes объединяется общим release change через `release_ref`
- **GIVEN** существует release change `release-2026-05-21-night`
...
- **THEN** каждый из них может ссылаться на `release-2026-05-21-night` через metadata-поле `release_ref`
