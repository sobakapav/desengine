...
- [x] 1. Зафиксировать роль `release-2026-05-21-night` как релизной метки без изменения `parent_change` у downstream changes.
- [x] 2. Описать состав ночной волны fixes/docs-fix и связь с triage пользовательских жалоб от `dispatcher-bugfix`.
- [x] 3. Добавить delta spec на уровне `admin-tools`, который закрепляет traceability релизной группировки через `release_ref`.
