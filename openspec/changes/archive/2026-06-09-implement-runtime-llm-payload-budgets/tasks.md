...
- [x] 1. Зафиксировать bounded contract для LLM input/output path.
- [x] 2. Ввести budget-проверки до дорогих побочных эффектов:
  - [x] 2.1 ограничить размер instruction и связанного runtime context;
  - [x] 2.2 ограничить input images и structured-output path;
  - [x] 2.3 ограничить итоговый write-set до записи пользовательских файлов.
- [x] 3. Провести budget-error через user-facing runtime:
  - [x] 3.1 oversized path завершается явной bounded ошибкой;
  - [x] 3.2 oversized output не приводит к частичной записи файлов;
  - [x] 3.3 runtime отличает budget-error от timeout/network/provider ошибок.
