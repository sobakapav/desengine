...
- `projects`: "Пользователь начинает работу над компонентом проекта", "Компонент проекта сохраняет связь с workflow template".
- `workflow`: "Пользователь запускает workflow из компонента проекта".
- `testing-layer`: unit/source-contract проверки project-aware workflow boundary и component workflow surface.
