...
- Что должен изменить этот change: Убрать legacy runtime из product-пути проекта, перенести работу на уровень проекта и сделать project page главной рабочей поверхностью.
...
- Какие ещё файлы и спецификации обязательны к чтению для implement-project-product-page:
...
- Что входит в этот change:
  - project-owned session/state в canonical server-backed project storage;
  - project-first workspace, workflow readout и history surface;
  - project component actions `создать`, `взять в работу`, `показать в workflow`, `вернуть в работу`;
  - redirect legacy pre-project route в `/projects`;
  - обновление active OpenSpec и unit/source-contract tests под новую product-модель.
- Что сознательно не входит в этот change:
  - полное физическое удаление legacy runtime из репозитория;
  - новый project-owned workbench/editor page;
  - отдельный новый editor/runtime beyond project page.
...
- Что именно должен доказать результат проверки: project surface действительно перестаёт зависеть от legacy entry path, показывает project-owned workflow/history, а source-contract tests фиксируют redirect legacy routes и новую project-first модель.
...
- Какие вопросы исполнитель должен закрыть по ходу работы:
  ...
  - какие legacy-derived панели нужно убрать сразу, а какие можно оставить за пределами project path;
  - как показать несколько параллельных рабочих линий проекта без нового отдельного workbench route.
