...
describe("preview runtime contract message", () => {
  ...
  it("строит явную диагностику для блокировки check entrypoint при broken preview", () => {
    expect(getPreviewCheckGuardMessage({
      ...
      "Проверка результата временно недоступна: preview сломан и не может подтвердить рендер компонента. Сначала исправьте ошибку preview: Тестовая runtime-ошибка preview",
    )
  })
