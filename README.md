...
Onboarding-контент runtime читает из каталога `/onboarding`, а его канонический внешний источник задаётся через `ONBOARDING_REPO_URL`. На первом запуске система пытается синхронизировать этот каталог автоматически; если статус остаётся проблемным, repair path находится на `/system`.
